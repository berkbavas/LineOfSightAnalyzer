    ...
    if (const auto it = FAILURE_TO_BEHAVIOUR.find(type); it != FAILURE_TO_BEHAVIOUR.end())
    {
        ...
        switch (behaviour)
        {
            ...
            case BehaviourType::ASSERT_OR_EXIT:
                ASSERT_OR_EXIT(false, "FailureBehaviour::Failure: An unrecoverable failure occured. Exiting...");
                break;
            default:
                ASSERT_OR_EXIT(false, "FailureBehaviour::Failure: Could not find BehaviourType for corresponding FailureType.");
                break;
        }
    }
    else
