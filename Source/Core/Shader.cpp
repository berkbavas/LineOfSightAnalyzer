...
void LineOfSightAnalyzer::Shader::Initialize()
{
    ...
    for (const auto& [ShaderType, Path] : mPaths)
    {
        ...
        if (Bytes.size() == 0)
        {
            LOS_EXIT_FAILURE("Shader::Initialize: '{}' could not be found: '{}'", GetShaderTypeString(ShaderType).toStdString(), mName.toStdString());
        }

        if (!mProgram->addShaderFromSourceCode(ShaderType, Bytes))
        {
            LOS_EXIT_FAILURE("Shader::Initialize: '{}' could not be loaded: '{}'", GetShaderTypeString(ShaderType).toStdString(), mName.toStdString());
        }
    }
    ...
    if (!mProgram->link())
    {
        LOS_EXIT_FAILURE("Shader::Initialize: Could not link shader program: '{}'", mName.toStdString());
    }

    if (!mProgram->bind())
    {
        LOS_EXIT_FAILURE("Shader::Initialize: Could not bind shader program: '{}'", mName.toStdString());
    }
...
void LineOfSightAnalyzer::Shader::Unbind()
{
    mProgram->release();
}

void LineOfSightAnalyzer::Shader::SetCallbackBeforeLinking(const Callback& Callback)
{
    mCallback = Callback;
}
...
void LineOfSightAnalyzer::Shader::SetSampler(const QString& Name, GLuint Unit, GLuint TextureId, GLuint Target)
{
