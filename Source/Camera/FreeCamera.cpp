...
bool LineOfSightAnalyzer::FreeCamera::OnKeyPressed(QKeyEvent* pEvent)
{
    mPressedKeys[(Qt::Key) pEvent->key()] = true;
    return false; // Let the event propagate
}

bool LineOfSightAnalyzer::FreeCamera::OnKeyReleased(QKeyEvent* pEvent)
{
    mPressedKeys[(Qt::Key) pEvent->key()] = false;
    return false; // Let the event propagate
}

bool LineOfSightAnalyzer::FreeCamera::OnMousePressed(QMouseEvent* pEvent)
{
...
bool LineOfSightAnalyzer::FreeCamera::OnMouseReleased(QMouseEvent* pEvent)
{
    if (pEvent->button() == Qt::MiddleButton)
    {
...
bool LineOfSightAnalyzer::FreeCamera::OnMouseMoved(QMouseEvent* pEvent)
{
...
bool LineOfSightAnalyzer::FreeCamera::IsKeyPressed(Qt::Key Key) const
{
    return mPressedKeys.value(Key, false);
}

float LineOfSightAnalyzer::FreeCamera::CalculateLinearSpeed(float Ifps) const
{
...
float LineOfSightAnalyzer::FreeCamera::CalculateAngularSpeed(float Ifps) const
{
    return mAngularSpeed * Ifps;
}

void LineOfSightAnalyzer::FreeCamera::ApplyRotation(float YawDelta, float PitchDelta)
{
    mYaw -= YawDelta;
    mPitch -= PitchDelta;
