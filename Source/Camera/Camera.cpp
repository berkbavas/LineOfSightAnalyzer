...
QVector3D LineOfSightAnalyzer::Camera::ComputeRayFromScreen(const QPointF &Point) const
{
...
QVector2D LineOfSightAnalyzer::Camera::ProjectWorldToScreenSpace(const QVector3D &Pos3D) const
{
...
QVector3D LineOfSightAnalyzer::Camera::ProjectScreenToWorldSpace(const QPointF &Point, float Depth) const
{
    // Convert screen position to Normalized Device Coordinates (NDC)
    float NdcX = (2.0f * Point.x()) / mWidth - 1.0f;
    float NdcY = 1.0f - (2.0f * Point.y()) / mHeight;
...
float LineOfSightAnalyzer::Camera::GetDepthOfPoint(const QVector3D &Point) const
{
    QMatrix4x4 ViewProjectionMatrix = GetViewProjectionMatrix();
...
QMatrix4x4 LineOfSightAnalyzer::Camera::GetViewProjectionMatrix() const
{
    return GetProjectionMatrix() * GetViewMatrix();
}

QMatrix4x4 LineOfSightAnalyzer::Camera::GetViewMatrix() const
{
...
QMatrix4x4 LineOfSightAnalyzer::Camera::GetRotationMatrix() const
{
...
QVector3D LineOfSightAnalyzer::Camera::GetViewDirection() const
{
    return GetRotation() * NEGATIVE_Z; // The view direction is the negative Z axis in the camera's local space
}

QVector3D LineOfSightAnalyzer::Camera::GetDirection(ViewDirection ViewDirection) const
{
...
int LineOfSightAnalyzer::Camera::GetWidth() const
{
    return mWidth;
}

int LineOfSightAnalyzer::Camera::GetHeight() const
{
    return mHeight;
}

float LineOfSightAnalyzer::Camera::GetZNear() const
{
    return mZNear;
}

float LineOfSightAnalyzer::Camera::GetZFar() const
{
    return mZFar;
}

void LineOfSightAnalyzer::Camera::SetZNear(float ZNear)
{
    mZNear = ZNear;
}

void LineOfSightAnalyzer::Camera::SetZFar(float ZFar)
{
    mZFar = ZFar;
}

QVector2D LineOfSightAnalyzer::Camera::GetViewportSize() const
{
