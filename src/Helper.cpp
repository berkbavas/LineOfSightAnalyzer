...
LineOfSightAnalyzer::Helper::Helper() {}

QByteArray LineOfSightAnalyzer::Helper::GetBytes(QString path)
{
    QFile file(path);
...
QQuaternion LineOfSightAnalyzer::Helper::RotateX(float angleRadians)
{
    return QQuaternion::fromAxisAndAngle(QVector3D(1, 0, 0), qRadiansToDegrees(angleRadians));
}

QQuaternion LineOfSightAnalyzer::Helper::RotateY(float angleRadians)
{
    return QQuaternion::fromAxisAndAngle(QVector3D(0, 1, 0), qRadiansToDegrees(angleRadians));
}

QQuaternion LineOfSightAnalyzer::Helper::RotateZ(float angleRadians)
{
    return QQuaternion::fromAxisAndAngle(QVector3D(0, 0, 1), qRadiansToDegrees(angleRadians));
}

QQuaternion LineOfSightAnalyzer::Helper::Invert(const QQuaternion& rotation)
{
...
float LineOfSightAnalyzer::Helper::AngleBetween(const QVector3D& v1, const QVector3D& v2, const QVector3D& left)
{
...
void LineOfSightAnalyzer::Helper::GetEulerDegrees(const QQuaternion& rotation, float& yaw, float& pitch, float& roll)
{
    ...
    roll = LineOfSightAnalyzer::Helper::AngleBetween(xAxis, xAxisProj, left);

    if (yaw < 0.0f)
        yaw += 360.0f;
}

QVector3D LineOfSightAnalyzer::Helper::GetEulerDegrees(const QQuaternion& rotation)
{
    float yaw, pitch, roll;
...
QQuaternion LineOfSightAnalyzer::Helper::ConstructFromEulerDegrees(float yaw, float pitch, float roll)
{
